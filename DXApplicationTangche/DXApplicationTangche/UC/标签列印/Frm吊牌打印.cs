            ...
            if (this.logdt.Rows.Count != 0)
            {
                for (int on = 0; on < ordernumber; on++)
                {
                    try
                    {
                        ...
                        SQLmtm.DoInsert("a_product_log_p", new string[] { "ORDER_ID", "CUSTOMER_ID", "SHOP_ID", "SHOP_NAME", "STYLE_ID", "ORDER_NUMBER", "ORDER_DATE", "STYLE_NAME_CN", "SYTLE_YEAR", "SYTLE_SEASON", "REF_STYLE_ID", "SYTLE_FABRIC_ID", "MATERIAL_NAME_CN", "MATERIAL_COLOR", "STYLE_PUBLISH_CATEGORY_CD", "ORDER_NO", "LOG_ID" }, new string[] { this.logdt.Rows[0]["ORDER_ID"].ToString(), this.logdt.Rows[0]["CUSTOMER_ID"].ToString(), this.logdt.Rows[0]["shop_id"].ToString(), this.logdt.Rows[0]["shop_name"].ToString(), this.logdt.Rows[0]["style_id"].ToString(), this.logdt.Rows[0]["ORDER_NUMBER"].ToString(), this.logdt.Rows[0]["ORDER_DATE"].ToString(), this.logdt.Rows[0]["STYLE_NAME_CN"].ToString(), this.logdt.Rows[0]["SYTLE_YEAR"].ToString(), this.logdt.Rows[0]["SYTLE_SEASON"].ToString(), this.logdt.Rows[0]["REF_STYLE_ID"].ToString(), this.logdt.Rows[0]["SYTLE_FABRIC_ID"].ToString(), this.logdt.Rows[0]["MATERIAL_NAME_CN"].ToString(), this.logdt.Rows[0]["MATERIAL_COLOR"].ToString(), this.logdt.Rows[0]["STYLE_PUBLISH_CATEGORY_CD"].ToString(), this.logdt.Rows[0]["ORDER_NO"].ToString(), logid });
